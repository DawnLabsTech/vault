...
- USDC -> dawnSOL（オンチェーン、Jupiter）+ SOL-PERP ショートを並列実行
  - dawnSOL のステーキング報酬 ~7% がロングレグに自動上乗せ
  - レバレッジなし（1x 固定、perp 側の清算リスク実質ゼロ）
  - SOL ファンディングレートが十分プラスの時のみ稼働
  - **Perp 取引所** は `config.perp.exchange` で切り替え可能: `"binance"`（Binance Futures、デフォルト）または `"bulk"`（Bulk Trade オンチェーン Perp、テストネットのみ）
...
│   ├── bulk/              # REST + WebSocket クライアント（Bulk Trade Perp、テストネット）
...
### Bulk Trade SOL Perp（2026-04 調査・テストネット統合済み）

DN 戦略のショートレグとして [Bulk Trade](https://bulk.trade) を評価し、フルコネクターを実装。Bulk はバリデーターノードに照合エンジンを直接組み込む（"Bulk Tile"）Solana ネイティブの perp DEX で、非カストディアル決済で CEX 級のパフォーマンス（照合 ~20ms、ファイナリティ ~40ms）を目標とする。

**ステータス:** アルファネット（テストネット）のみ稼働中。メインネットは 2025 Q4 予定だったが遅延中、確定日未定。シード調達 $8M（6th Man Ventures, Robot Ventures リード、Anatoly Yakovenko 個人投資家として参加）。

**統合状況:** テストネット向けコネクターを実装・検証済み。`config.perp.exchange` を `"bulk"` に設定するだけで Binance と切り替え可能。起動時に `buildBulkDnConnectors()` が選択され、他のコード変更は不要。

| ファイル | 役割 |
|---|---|
| `bot/src/connectors/bulk/rest.ts` | REST クライアント（注文・証拠金残高・mark price・FR 取得）|
| `bot/src/connectors/bulk/ws.ts` | WebSocket クライアント（リアルタイム mark price、自動再接続）|
| `bot/src/connectors/bulk/keychain.ts` | Bulk の認証方式向け Ed25519 署名ラッパー |
| `bot/src/connectors/bulk/types.ts` | Bulk API の TypeScript 型定義 |
| `bot/scripts/test-bulk-flow.ts` | テストネット統合検証スクリプト（status / funding / faucet / short / close）|

| 項目 | Binance（現行）| Bulk Trade |
|---|---|---|
| ステータス | 本番稼働 | アルファネット（テストネット）|
| 手数料 | maker 0.02% / taker 0.04% | maker 0-0.02% / taker 0.022-0.035% |
| 最大レバレッジ | 125x | 20x |
| 担保 | USDT/USDC | USDC のみ |
| 証拠金 | クロス / アイソレート | ポートフォリオ証拠金（相関調整）|
| カストディ | CEX（取引所管理）| 非カストディアル（Solana スマートコントラクト）|
| KYC | 必要 | 不要 |

**Vault にとってのメリット:**
- 非カストディアル — CEX カウンターパーティリスクを排除
- KYC 不要 — ファンド運用の規制対応コストを削減
- 手数料優位（メイカーリベートあり）
- 1h 資金調達決済でヘッジ管理の精度向上
- ポートフォリオ証拠金によるマージン効率 30-70% 改善
- Solana ネイティブ — クロスチェーンなしで USDC 担保

**リスク:**
- メインネット未稼働 — 本番実績なし
- 流動性ゼロ — オーダーブックの深さとスリッページ未知
- 公式セキュリティ監査未公開
- Solana アップグレード互換性リスク

**判断:** メインネット移行は保留。テストネットコネクターは準備済み。本番移行はメインネットローンチ・セキュリティ監査完了・十分な流動性確認後に再検討。
